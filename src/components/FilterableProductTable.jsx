...
export default class FilterableProductTable extends React.Component {
  ...
  render() {
      ...
      return (
        <DropdownItem>
          <NavLink tag={Link} to={cityLink}>
            <button onClick={this.handleClick}>{city}</button>
          </NavLink>
        </DropdownItem>
      );
    });
    ...
    const routeList = tags.map((tag) => {
      let link = "/" + tag;
      return (
        <Route exact path={link} render={() => <SpotTable city={tag} />} />
      );
    });
    ...
    return (
      <Router>
        <Navbar color="faded" light expand="md">
          ...
          <Collapse navbar>
            <Nav navbar>
              ...
              <UncontrolledDropdown nav inNavbar className="col-auto">
        ...
        <footer className="container">
          <div className="row">
            <span className="text-muted text-center col">Dcard Intern.</span>
          </div>
        </footer>
      </Router>
    );
